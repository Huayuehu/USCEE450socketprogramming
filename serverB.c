...
char buf[BUFFER_SIZE];
	FILE *fp; 
	fp = fopen("database_b.csv", "r"); // reading only open file "database_b.csv"
...
int main(void){
    ...
    */
	...
	memset(&hints, 0, sizeof hints);  	// mset struct to all zero
...
// loop through all the results and bind to the first we can----from Beej
	...
	printf( "\nThe Server B is up and running using UDP on port <%s>.\n", MYPORT);
